...
module Globs
	...
	def self.run_application
		begin
			Threads.run_main_handle
			Threads.run_notes_update_handle
			Threads.run_thread_filter_handle
  ...
  def self.main
	...
	def self.read_files_recursive(dir)
		...
		Dir.foreach(dir) { |file|
			next if file == "." || file == ".."
			begin
				Globs.read_files_recursive(dir + "/" + file) if file.match(/\./).nil?
			rescue Errno::ENOENT
				# Supplement Errno::ENOENT exception
				# If unable to open file, display message and end
				filename = $!.message.sub("No such file or directory - ", "")
